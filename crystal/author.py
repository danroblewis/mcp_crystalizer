...
def _record_of(output, run_dir: Path | None = None) -> dict | None:
    ...
    if isinstance(output, dict) and output.get("run_id"):
        p = (run_dir or RUN_DIR) / f"{output['run_id']}.json"
        if p.exists():
...
def expand_flow_calls(session: Session, run_dir: Path | None = None) -> tuple[Session, list[str]]:
    """Replace each flows.run_flow call by the calls that flow made; drop flows.list_flows and Claude Code's own
    Read/Grep/Glob (a flow cannot run those). Returns the new session and the names of the flows it ran."""
    calls, ran = [], []
    for c in session.calls:
        if c["server"] == "flows":
            if c["tool"] == "run_flow":
                rec = _record_of(c.get("output"), run_dir)
                if rec:
                    ran.append(rec["flow"])
...
def induce_version(trigger: str, base: str, new_session: Session, trace_dir: Path | None = None, flow_dir: Path | None = None,
    ...
    others = [expand_flow_calls(s, run_dir)[0] for s in load_sessions(trace_dir, trigger=trigger) if s.session_id != new_session.session_id]
...
def author(trigger: str, inputs: dict, budget="3", model=None, name: str | None = None, driver_fn=run_agent,
    ...
    expanded, ran = expand_flow_calls(session, run_dir)
